...
def compare_exact(genome_fasta, ref_fasta, query_fasta, caller_type, min_size, ggcaller_version):
    # generate lists to hold gene coordinates
    genome_list = []
    genome_rec = {}
    ref_rec = {}
...
def main():
    ...
    if options.caller not in ("ggc", "prod"):
        ...
        if options.caller == "ggc" and (options.vers < 0 or options.vers > 1.3):
            ...
            if options.exact:
                output_tuple = compare_exact(options.seq, options.genes, options.query, options.caller,
                                              options.min_size, options.vers)
            else:
